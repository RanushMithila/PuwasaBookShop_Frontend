...
const useBillingStore = create((set, get) => ({
  ...
  addItem: (item) => set((state) => {
    ...
    if (existingIndex !== -1) {
      ...
      return { selectedItems: [...state.selectedItems, { ...item, QTY: initialQty, Discount: initialDiscount }] };
    }
  }),
  ...
  updateItemDiscount: (inventoryID, discount) => set((state) => ({
    ...
    selectedItems: state.selectedItems.map((item) => {
      ...
      const safeDiscount = Math.max(0, Math.min(lineTotal, parseFloat(discount) || 0));
