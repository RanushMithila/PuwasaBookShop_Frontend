...
const BillingItemRow = ({ item, onDoubleClick, registerRowRef, onFocusItemCode }) => {
  ...
  return (
    ...
    >
      {/* Item Code column */}
      <div className="pr-2 flex items-center text-sm text-gray-700">
        <div className="truncate">{item.barcode || item.inventoryID || 'N/A'}</div>
      </div>
      ...
      {/* Unit Price column */}
  <div className="flex items-center font-medium text-gray-800">Rs: {(item.itemUnitPrice || 0).toFixed(2)}</div>
      ...
      <div className="flex items-center font-semibold text-gray-900">
        ...
        {discountAmount > 0 && (
          <span className="text-xs text-emerald-700 ml-1">(-Rs: {discountAmount.toFixed(2)})</span>
        )}
      </div>
